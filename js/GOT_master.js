(() => {
	...
	const 	sigils 		= document.querySelectorAll('.sigil-container'),
			...
			clightbox 	= document.querySelector('.closelightbox'),
			banner 		= document.querySelector('#houseImages'),
			nameHouse	= document.querySelector('.namehouse'),
			infoHouse	= document.querySelector('.infohouse');
	...
	function OpenLightBox() {
		...
		let houseTarget = this.className.split(" ")[1];
		
		targetSrc=houseTarget.charAt(0).toUppperCase() + houseTarget.slice(1);
	...
	function bannermovement() {
		...
		nameHouse.textContent = `House ${dataforhouse[this.dataset.offset][0]}`;
		infoHouse.textContent = dataforhouse[this.dataset.offset][1];
		TweenMax.to(banner, 0.8, { right: totalOffset });
	}

	sigils.forEach(sigil => sigil.addEventListener('click', OpenLightBox));
	sigils.forEach(sigil => sigil.addEventListener('click', bannermovement));

	video.addEventListener('ended', CloseLightBox);
	clightbox.addEventListener('click', CloseLightBox);
